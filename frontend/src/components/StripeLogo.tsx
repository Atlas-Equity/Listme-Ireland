...
interface StripeLogoProps {
  className?: string;
  variant?: 'blurple' | 'white' | 'dark' | 'currentColor';
  height?: number;
}
...
export function StripeLogo({ 
  ...
  // Exact vector paths from official Stripe Brand Asset (2.4:1 ratio)
  return (
    <svg 
...
export function StripeBadge({ 
  ...
  if (variant === 'secured') {
    return (
      <span className={`inline-flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 font-medium ${className}`}>
        <span>Secured by</span>
        <StripeLogo height={height} variant="blurple" />
      </span>
    );
  }
  ...
  if (variant === 'solid') {
    return (
      <span className={`inline-flex items-center px-2 py-0.5 rounded-full bg-[#635BFF] text-white shadow-2xs ${className}`}>
        <StripeLogo height={height} variant="white" />
      </span>
    );
  }

  // 'pill' default (Stripe blurple tinted pill with official Stripe logo)
  return (
    <span 
      className={`inline-flex items-center px-2 py-0.5 rounded-md bg-[#635BFF]/10 dark:bg-[#635BFF]/20 border border-[#635BFF]/30 transition-colors ${className}`}
      title="Payments powered securely by Stripe"
    >
      <StripeLogo height={height} variant="blurple" />
    </span>
  );
